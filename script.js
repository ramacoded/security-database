...
const GITHUB_TOKEN = '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'
...
const dotsButton = document.createElement('button') dotsButton.className = 'dots-button' dotsButton.innerHTML = '&#8942'
...
try {
const response = await fetch(`https://api.github.com/repos/${GITHUB_USERNAME}/${REPO_NAME}/contents/${FILE_PATH}`, { headers: { 'Authorization': `token ${GITHUB_TOKEN}` } })
if (response.status === 404) { numbersData = [] fileSha = null }
else if (response.ok) { const data = await response.json() numbersData = JSON.parse(atob(data.content)) fileSha = data.sha }
...
try {
const content = btoa(JSON.stringify(numbersData, null, 2))
const body = { message: `Update ${FILE_PATH}`, content, sha: fileSha }
const response = await fetch(`https://api.github.com/repos/${GITHUB_USERNAME}/${REPO_NAME}/contents/${FILE_PATH}`, { method: 'PUT', headers: { 'Authorization': `token ${GITHUB_TOKEN}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
if (response.ok) { const data = await response.json() fileSha = data.content.sha renderNumbers(numbersData) }
else { throw new Error(`GitHub API Error: ${response.statusText}`) }
} catch (error) { alert(`Failed to save data: ${error.message}`) await loadNumbers() }
finally { hideLoader() }
}
...
passActual.textContent = APP_PASSWORD
